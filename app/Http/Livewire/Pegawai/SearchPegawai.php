...
class SearchPegawai extends Component
{
    ...
    public function updatedSelect($value)
    {
        ...
        if ($pegawai) {
            ...
            $this->selectedSekolah = $pegawai->asn->sekolah;
    ...
    {
        $this->results = Pegawai::where('status_tenaga', 'asn')->whereHas('asn', function ($query) {
            $query->where('jenis_tenaga', 'nakes');
        })->get();
